...
function ComparePage() {
  const [selectedGame1, setSelectedGame1] = useState(mockGames[0].id);
  const [selectedGame2, setSelectedGame2] = useState(mockGames[1].id);
  const [selectedGame3, setSelectedGame3] = useState<number | null>(null);

  const game1 = mockGames.find(g => g.id === selectedGame1);
  const game2 = mockGames.find(g => g.id === selectedGame2);
  const game3 = selectedGame3 ? mockGames.find(g => g.id === selectedGame3) : null;
